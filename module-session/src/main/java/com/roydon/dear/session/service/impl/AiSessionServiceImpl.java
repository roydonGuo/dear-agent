...
public class AiSessionServiceImpl extends ServiceImpl<AiSessionMapper, AiSession> implements AiSessionService {
    ...
    public boolean updateAnswer(UpdateAnswerRequest request) {
        ...
        if (session != null) {
            ...
            if (request.getThinking() != null) session.setThinking(request.getThinking());
            if (request.getTools() != null) session.setTools(request.getTools());
            if (request.getReference() != null) session.setReference(request.getReference());
            if (request.getFirstResponseTime() != null) session.setFirstResponseTime(request.getFirstResponseTime());
            if (request.getTotalResponseTime() != null) session.setTotalResponseTime(request.getTotalResponseTime());
            if (request.getRecommend() != null) session.setRecommend(request.getRecommend());
